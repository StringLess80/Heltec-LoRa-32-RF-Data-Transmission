...
* **Architettura Modulare e Separazione delle Responsabilità:** Codice Python riorganizzato in pacchetti modulari (`config`, `geo`, `tracker`, `ui`, `evaluator`, `encoder`, `fields_selector`) per massima manutenibilità e pulizia.
...
* **Sincronizzazione e Scarto Selettivo:** In caso di corruzione di un pacchetto sulla seriale in ricezione (`python/rx/`), il parser scarta unicamente il marcatore di sincronizzazione `0x55AA` corrente e riallinea immediatamente il puntatore del buffer ai byte successivi, minimizzando la perdita di pacchetti adiacenti.
...
Il ricevitore (`python/rx/`) include l'implementazione in virgola mobile della formula di Haversine per determinare la distanza ortodromica in tempo reale tra ciascun velivolo rilevato e le coordinate geografiche della stazione di terra (`HOME_LAT`, `HOME_LON`):
...
### Componenti software (Architettura Modulare Python)

#### `python/tx/` (Modulo Trasmettitore)
* **`config.py`**: Definizione costanti, soglie delta, temporizzazioni, strutture binarie precompilate e mappe campi SBS.
* **`fields_selector.py`**: Interfaccia CLI interattiva per la selezione dei campi da trasmettere.
* **`evaluator.py`**: Motore Semantico (confronto delta ed eventi FULL/DELTA/EVENT).
* **`encoder.py`**: Serializzatore binario compresso ad alte prestazioni con numero di sequenza e CRC32.
* **`tracker.py`**: Parsing veloce delle righe SBS da Dump1090/readsb e gestione memoria aerei con Garbage Collection.
* **`tx.py`**: Entry point del trasmettitore: connessione al socket TCP, gestione porta seriale e coda TX asincrona.

#### `python/rx/` (Modulo Ricevitore)
* **`config.py`**: Parametri geografici della stazione base (Home), timeout e definizioni delle strutture di decodifica binaria.
* **`geo.py`**: Calcolo della distanza ortodromica in tempo reale tramite formula di Haversine.
* **`input_handler.py`**: Gestore input tastiera non bloccante e multipiattaforma (Windows/Linux/macOS).
* **`tracker.py`**: Decodificatore dei pacchetti binari LoRa e tracciamento dello stato degli aerei attivi.
* **`ui.py`**: Interfaccia grafica TUI interattiva basata sulla libreria `Rich` (card velivoli, griglia e scrolling).
* **`rx.py`**: Entry point del ricevitore: sincronizzazione seriale su marcatore `0x55AA`, validazione CRC e loop grafico.
...
Spostati nella directory del ricevitore ed esegui lo script principale:
...
Apri un altro terminale sulla macchina connessa all'SDR, spostati nella directory del trasmettitore e avvia lo script principale:
